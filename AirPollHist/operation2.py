...
df_Beijing = pd.read_csv('/home/ankesh/Desktop/AQI-Air-quality-index/BeijingPM2.5/FiveCitiePMData/BeijingPM20100101_20151231.csv')
...
def main(StringData):
    ...
    if rf_test_predict>0 and rf_test_predict<=50:
    	print("PM 2.5 Level:%s->No Health Implications"%(rf_test_predict))
    elif rf_test_predict>50 and rf_test_predict<=100:
    	print("PM 2.5 Level:%s->Hypersensitive individuals should reduce outdoor Activities"%(rf_test_predict))
    elif rf_test_predict>100 and rf_test_predict<=150:
    	print("PM 2.5 Level:%s->Individual with breathing or heart problem should reduce outdoor Activities"%(rf_test_predict))
    elif rf_test_predict>150 and rf_test_predict<=200:
    	print("PM 2.5 Level:%s->Individual with breathing or heart problem should reduce outdoor Activities"%(rf_test_predict))
    elif rf_test_predict>200 and rf_test_predict<=300:
    	print("PM 2.5 Level:%s->Individual with breathing or heart problem should avoid outdoor Activities"%(rf_test_predict))
    elif rf_test_predict>300 and rf_test_predict<=500:
    	print("PM 2.5 Concentration is:%s-->Healthy Individual should avoid outdoor Activities"%(rf_test_predict))
